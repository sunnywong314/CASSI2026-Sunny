...
100    format(F8.6)
  ...
  subroutine AESOPUS_interp(Zref, X, XC, XN, XO, logR, logT, res, ierr)
    ...
    logical :: clipped
    real(dp) :: logT_to_use, logR_to_use
    ...
    if (outside_R_and_T_bounds(logR,logT)) then
       ...
       ! ierr = -1
       ! return
       if (logR > kA% max_logR) logR_to_use = kA% max_logR
       if (logR < kA% min_logR) logR_to_use = kA% min_logR

       if (logT < kA% min_logT) logT_to_use = kA% min_logT
       if (logT > kA% max_logT) logT_to_use = kA% max_logT
    end if
    ...
    if (clipped) then
       call AESOPUS_interp_fixedZref(iZ, X, XC, XN, XO, logR_to_use, logT_to_use, res, ierr)
       if ((logR > kA% max_logR) .or. (logR < kA% min_logR)) res(2)= 0d0 ! d_dlogR = 0 if extrapolated
       if ((logT > kA% max_logT) .or. (logT < kA% min_logT)) res(3)= 0d0 ! d_dlogT = 0 if extrapolated
       return
    end if
    ...
    do i = 1, nZ
       call AESOPUS_interp_fixedZref(iZ+i-2, X, XC, XN, XO, logR_to_use, logT_to_use, raw_res(:,i), ierr)
    end do
